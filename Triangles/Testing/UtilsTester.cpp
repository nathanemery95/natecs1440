...
void UtilsTester::testSplit()
{
    ...
    lineToSplit = "ABCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCDDDDDDDDDD";// ,D,E
    if (split(lineToSplit, ',', pieces, 1)) {//should be 3 peices
    ...
    if (split(lineToSplit, ',', pieces, 0)) { // should be 3 peices
        if (pieces[0] != "ABCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCDDDDDDDDDD" || pieces[1] != "D" || pieces[2] != "E") {///mno, pqr, stu
    ...
    // Check bad cases
    lineToSplit = "MNO,PQR,STU";
    if (split(lineToSplit, ',', pieces, 4)) {// pieces should be 3 not 4
        std::cout << "Failure in split(lineToSplit, ',', pieces, 0) for lineToSplit=\""
                  << lineToSplit << "\": result value not as expected" << std::endl;
        return;
    }
